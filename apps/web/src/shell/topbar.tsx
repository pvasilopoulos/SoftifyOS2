...
export function Topbar() {
  ...
  return (
    <header className="flex h-12 shrink-0 items-center gap-2 border-b border-line px-3 md:h-14 md:gap-3 md:px-4">
      <div className="min-w-0 flex-1 md:flex-none">
        <div className="hidden text-[11px] font-medium uppercase tracking-[0.14em] text-faint sm:block">
          {org.name}
        </div>
        <div className="truncate text-sm font-semibold">{key ? t.nav[key] : t.brand}</div>
      </div>
      <IconButton label={t.search} onClick={() => setCommandOpen(true)} className="sm:hidden">
        <Search className="size-4" />
      </IconButton>
      <button
        type="button"
        onClick={() => setCommandOpen(true)}
        className="mx-auto hidden h-9 w-full max-w-md items-center gap-2 rounded-xl border border-line bg-bg-1 px-3 text-left text-sm text-faint transition hover:border-line-strong sm:flex"
      >
